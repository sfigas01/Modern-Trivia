...
| Metric                 | Value                                                                                                                                                                  | Notes                                                       |
| :--------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------- |
| **Parent Epic**        | [EPIC-01: Dispute Resolution](epics/EPIC-01_dispute_resolution.md)                                                                                                     | _Technically supports Dispute Res but enables future Scale_ |
| **Benefit Hypothesis** | As an **Admin**, I want to **search, filter, and mass-edit questions**, so that I can **manage data quality and find duplicates** as the database grows to 10k+ items. | _Critical for scalability._                                 |
| **WSJF Score**         | [High User Value] + [Med Time Criticality] / [Med Job Size] = **High**                                                                                                 | _Essential before we add more content._                     |
...
_> **Goal:** Enable Admins to manage the question lifecycle at scale._
...
- [ ] **Task 1:** Backend Search API
  - _Implementation:_ Create/Update `GET /api/questions` to accept filters.
  - _Verification:_ `curl` request with search term returns filtered subset.
- [ ] **Task 2:** Inventory Table UI
  - _Implementation:_ Build React table page with search bar and difficulty dropdown.
  - _Verification:_ Verify sorting works on headers.
- [ ] **Task 3:** Edit Modal Integration
  - _Implementation:_ Reuse "Edit Question" form in a modal when clicking a row.
  - _Verification:_ Saving updates the list row immediately.
